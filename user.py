...
def select_users(con, no_of_user = None):  #6  number
    ...
    if no_of_user:
       users= cur.execute("SELECT * FROM users LIMIT ?;", (no_of_user,))
        ...
        users = cur.execure("SELECT * FROM users;")
    for user in users:
      print(user)
...
def delete_user_by_id(con, user_id):  #8 number
   ...
   con.commit()
   print("user with id [{user_id}] was deleted successfully.")

COLUMNS = (   #3 number ko lagi agi pass garera gareko wala aba user sanga liyera gareko
      "first_name",
      "last_name",
      "company_name",
      "address",
...
def main():
   ...
   if user_input=="1":
      ...
      no_of_user = input("How many users do you want to see:")
